...
function nextTypingIndex(index: number, target: string, itemText: string, isVocabulary: boolean) {
  const nextIndex = index + 1;
  const isRepeatSeparator = isVocabulary && itemText.length > 0 && nextIndex < target.length
    && target[nextIndex] === " " && (nextIndex + 1) % (itemText.length + 1) === 0;
  return isRepeatSeparator ? nextIndex + 1 : nextIndex;
}
...
export default function Home() {
  ...
  useEffect(() => {
    if (status !== "playing" || level !== "VOCAB" || !audioEnabled || !currentItem || currentRepeat < 1) return;
    const autoPlayKey = `${vocabMode}:${currentItem.audioId}:${currentRepeat}`;
  ...
  }, [audioEnabled, currentItem, currentRepeat, level, playEntry, status, vocabMode]);
  ...
  const hitKey = (key: string) => {
    ...
    if (key === target[index]) {
      ...
      const nextIndex = nextTypingIndex(index, target, currentText, level === "VOCAB");
      if (nextIndex >= target.length) { if (level === "VOCAB") { setWordsCompleted((v) => v + 1); chooseEntry(currentItem); } else setTarget(makeBaseTarget(level as BaseLevel)); setIndex(0); } else setIndex(nextIndex);
